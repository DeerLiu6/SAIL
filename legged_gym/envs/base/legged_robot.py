...
from legged_gym.utils.isaacgym_utils import euler_from_quaternion
...
class LeggedRobot(BaseTask):
    ...
    def check_termination(self):
        ...
        reach_goal_cutoff = self.cur_goal_idx >= self.cfg.terrain.num_goals # all goals reached
        self.reset_buf |= reach_goal_cutoff

        height_cutoff = (self.root_states[:, 2] - 0.7) < -0.25 # foot height below -0.25 (fell into a pit)
    ...
    # Compute angular-velocity commands from target and heading, and measure terrain.
    ...
    def _resample_commands(self, env_ids):
        ...
        # set small commands to zero (only for lin_vel_x and lin_vel_y)
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        noise_vec[9+3*self.num_actions:9+3*self.num_actions+6] = 0.
        if self.cfg.terrain.measure_heights:
            noise_vec[57:211] = noise_scales.height_measurements* noise_level * self.obs_scales.height_measurements
        return noise_vec
    #----------------------------------------
    def _init_buffers(self):
        ...
        self.delta_yaw = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        self.delta_next_yaw = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _draw_debug_vis(self):
        ...
        sphere_geom = gymutil.WireframeSphereGeometry(0.02, 10, 10, None, color=(0, 1, 0))
        i = self.lookat_id
    ...
    # Sample terrain heights at specific points around the robot.
    # This parameter specifies environment IDs to return heights for; if empty, sample all environments.
    ...
    #------------ reward functions----------------
# root_states
# Index range  Data content                Description
# 0:3          Position (x, y, z)          Base position in world coordinates.
# 3:7          Quaternion (qx, qy, qz, qw) Base orientation in quaternion form.
# 7:10         Linear velocity (vx, vy, vz) Base linear velocity in world coordinates.
# 10:13        Angular velocity (wx, wy, wz) Base angular velocity in world coordinates.
    def _reward_tracking_goal_vel(self):
        norm = torch.norm(self.target_pos_rel, dim=-1, keepdim=True) # norm of the relative position vector
        target_vec_norm = self.target_pos_rel / (norm + 1e-5)  # normalize to a unit vector pointing to the target
        cur_vel = self.root_states[:, 7:9]
        # reward is based on velocity projected onto target direction, capped by the target speed
